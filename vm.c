...
void sweep(vm_t *vm) {
    ...
    // Iterate over all objects in the VM
    for (size_t i = 0; i < vm->objects->capacity; i++) {
        snek_object_t *obj = (snek_object_t *)vm->objects->data[i];
        if (obj == NULL) continue;

        if (obj->is_marked) {
            obj->is_marked = false; // unmark for next GC cycle
        } else {
            snek_object_free(obj);
            vm->objects->data[i] = NULL; // remove from stack
        }
    }

    // Remove NULLs from the objects stack
